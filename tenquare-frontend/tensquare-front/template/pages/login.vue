        ...
        <div class="form">
            ...
            <form class="sui-form">
                <div class="control-group">
                    ...
                    <div class="controls">
                        <input type="text" v-model="pojo.nickname" id="inputname" placeholder="真实姓名或常用昵称" class="input-xlarge" />
                    </div>
                    </div>
                    <div class="different">
        ...
        <div class="form">
            ...
            <form class="sui-form login-form">
                ...
                <div class="control-group">
                    <label for="inputpassword" class="control-label">密码：</label>
                    <div class="controls">
                        <input type="text" v-model="password" id="inputpassword" placeholder="输入登录密码" class="input-xlarge" />
                    </div>
                </div>
                <div class="controls">
...
export default {
    ...
    methods: {
        sendSMS() {
            return userApi.sendSMS(this.pojo.mobile).then(resp => {
                this.$message({
        ...
        register() {
            return userApi.register(this.pojo, this.code).then(resp => {
                this.$message({
                    message: resp.data.message,
                    type: (resp.data.flag?'success':'error')
                })
        ...
        login() {
            return userApi.login(this.mobile, this.password).then(resp => {
                if (resp.data.flag) {
                    Auth.setUser(resp.data.data.token, resp.data.data.nickName,
                        resp.data.data.uid, resp.data.data.image)
